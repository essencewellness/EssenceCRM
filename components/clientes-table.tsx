...
function formatDate(d: string | null) {
  if (!d) return "—"
  return new Intl.DateTimeFormat("pt-PT", { day: "numeric", month: "short", year: "numeric" }).format(new Date(d))
}
...
// NUIT tag system — sharp, uppercase, 9.5px, 0.32em tracking
const estadoMap: Record<string, { label: string; color: string; bg: string; border: string }> = {
  ...
  perdida:         { label: "Perdida",    color: "#7a7e8a", bg: "transparent",             border: "rgba(122,126,138,0.35)" },
      ...
      {clientes.map((cliente, idx) => {
        ...
        return (
          <motion.tr
            ...
            whileHover={{
              backgroundColor: "rgba(250,248,246,1)",
