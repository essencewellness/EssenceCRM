  ...
  return (
    <div style={{ maxWidth: "640px" }}>
      ...
      <div style={{
        ...
        <h2 style={{ fontFamily: "var(--font-heading, Georgia, serif)", fontSize: "calc(15px * var(--ui-font-scale))", color: "var(--nuit-midnight)", fontWeight: 400, marginBottom: "20px", paddingBottom: "12px", borderBottom: "1px solid #e8e2d9" }}>
          Sistemas Ligados
        </h2>
        {integracoes.map(int => (
          <div key={int.nome} style={{ padding: "14px 0", borderTop: "1px solid #f0ebe3", display: "flex", alignItems: "flex-start", gap: "16px" }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
                <p style={{ fontFamily: "var(--font-sans)", fontSize: "calc(13px * var(--ui-font-scale))", fontWeight: 600, color: "var(--nuit-midnight)" }}>{int.nome}</p>
                <span style={badgeStyle(int.estado)}>{int.estado}</span>
              </div>
              <p style={{ fontFamily: "var(--font-sans)", fontSize: "calc(12px * var(--ui-font-scale))", color: "var(--nuit-bone-soft)", lineHeight: 1.5, marginBottom: "4px" }}>{int.descricao}</p>
              {int.detalhe && (
                <p style={{ fontFamily: "monospace", fontSize: "calc(11px * var(--ui-font-scale))", color: "#9d9d9a" }}>{int.detalhe}</p>
              )}
            </div>
            {int.webhookUrl && (
              <button
                ...
                style={{
                  ...
                  fontSize: "calc(11px * var(--ui-font-scale))", color: "var(--nuit-bone-soft)", cursor: "pointer",
                }}
              >
            ...
            {resultados[int.nome] !== undefined && (
              <span style={{ fontFamily: "var(--font-sans)", fontSize: "calc(11px * var(--ui-font-scale))", color: resultados[int.nome] ? "#7a9e7e" : "var(--destructive)" }}>
      ...
      <div style={{
        ...
        <p style={{ fontFamily: "var(--font-sans)", fontSize: "calc(12px * var(--ui-font-scale))", color: "var(--nuit-bone-soft)", marginBottom: "16px", lineHeight: 1.6 }}>
          Para activar os webhooks, definir as variáveis de ambiente <code style={{ fontFamily: "monospace", fontSize: "calc(11px * var(--ui-font-scale))", backgroundColor: "rgba(212,184,134,0.08)", padding: "1px 4px", borderRadius: "2px" }}>WEBHOOK_N8N_*</code> no Vercel.
        </p>
          ...
          {webhookEventos.map(ev => {
            ...
            return (
              <div key={ev.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", backgroundColor: "#fff", border: "1px solid #e8e2d9", borderRadius: "3px" }}>
                <span style={{ fontFamily: "var(--font-sans)", fontSize: "calc(12px * var(--ui-font-scale))", color: "var(--nuit-midnight)" }}>{ev.label}</span>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <span style={{ fontFamily: "monospace", fontSize: "calc(10px * var(--ui-font-scale))", color: "#9d9d9a" }}>WEBHOOK_N8N_{ev.key.toUpperCase().replace(/\./g, "_")}</span>
