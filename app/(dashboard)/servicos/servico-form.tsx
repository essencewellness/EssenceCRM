...
export default function ServicoForm() {
  ...
  const inputStyle: React.CSSProperties = {
    padding: "8px 12px", borderRadius: "8px", border: "1px solid #ddd6c4",
  ...
  return (
    <div style={{ background: "rgba(185,160,122,0.06)", borderRadius: "12px", padding: "20px 24px", border: "1px solid rgba(185,160,122,0.2)" }}>
      <h2 style={{ fontFamily: "Georgia, serif", fontSize: "15px", color: "#3d2c1e", marginBottom: "16px" }}>Novo Serviço</h2>
      <form ref={formRef} onSubmit={handleSubmit} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
        <div style={{ gridColumn: "1 / -1" }}>
          <label style={{ fontSize: "12px", color: "#8a7460", display: "block", marginBottom: "4px" }}>Nome *</label>
          <input name="nome" style={inputStyle} required placeholder="Ex: Drenagem Linfática" />
        </div>
        <div>
          <label style={{ fontSize: "12px", color: "#8a7460", display: "block", marginBottom: "4px" }}>Preço Base (€) *</label>
          <input name="precoBase" style={inputStyle} type="number" step="0.01" min="0" required placeholder="40" />
        </div>
        <div>
          <label style={{ fontSize: "12px", color: "#8a7460", display: "block", marginBottom: "4px" }}>Duração (min)</label>
          <input name="duracaoMinutos" style={inputStyle} type="number" min="5" max="480" defaultValue="60" />
        </div>
        <div style={{ gridColumn: "1 / -1" }}>
          <label style={{ fontSize: "12px", color: "#8a7460", display: "block", marginBottom: "4px" }}>Descrição</label>
